...
use app\modules\mch\models\RechargeCardForm;
use app\modules\mch\models\RechargeCardListForm;
use app\modules\mch\models\UserCardMiForm;
...
class UserController extends Controller
{
    ...
    {
        $form = new RechargeCardListForm();
        $form->store_id = $this->store->id;
        $form->attributes = \Yii::$app->request->get();
        $arr = $form->search();
        return $this->render('recharge_card', [
            'list' => $arr['list'],
            'pagination' => $arr['p'],
            'row_count' => $arr['row_count'],
        ]);
     ...
     * 充值卡删除
     */
    public function actionRechargeCardDel($id = null)
    {
        $rechargecard = RechargeCard::findOne(['id' => $id, 'store_id' => $this->store->id]);
        if (!$rechargecard) {
            return [
                'code' => 1,
                'msg' => '充值卡不存在',
...
//            ];
//        }
        $rechargecard->is_delete = 1;
        if ($rechargecard->save()) {
            $rechargecard->delete();
            return [
                'code' => 0,
                'msg' => '成功',
    ...
    //卡密生成
    public  function  actionRechargeCardMake($id){

        $arrs = array();
        for ($x=0; $x<=100; $x++) {
            $arrs[$x][] = $id;
            $arrs[$x][] =  $this->store->id;
            $arrs[$x][] = $this->random_card();
            $arrs[$x][] = $this->random_code();
            $arrs[$x][] = time();
        }

        $query = \Yii::$app->db->createCommand();
        $insert = $query->batchInsert(Cardmi::tableName(), ['level_type', 'store_id','card_id','card_password','addtime'], $arrs)->execute();
        if ($insert) {
            return [
                'code' => 0,
                'msg' => '成功',
    ...
    public function random_card($length = 4,$chars = null){
        $str = "123456789";
        return substr(str_shuffle($str), 0,$length). substr(strval(rand(10000,19999)),1,4);;

    }
    public function random_code($length = 6,$chars = null){
        if(empty($chars)){
     ...
     * 会员卡号列表
     */
    public function actionRechargeCardPassword($id = null)
    {

        $form = new UserCardMiForm();
        $exportList = $form->excelFields();
        $form->attributes = \Yii::$app->request->get();
        $form->attributes = \Yii::$app->request->post();
        $form->store_id = $this->store->id;
        ...
        $data = $form->search();
        $rechargecard_list = RechargeCard::find()->where(['store_id' => $this->store->id, 'is_delete' => 0, 'status' => 1])
            ->orderBy(['id' => SORT_ASC])->asArray()->all();
//        echo "<pre>";
//        print_r($rechargecard_list);die;

        // excel导出

        return $this->render('recharge-card-password', [
            'row_count' => $data['row_count'],
            'pagination' => $data['pagination'],
            'list' => $data['list'],
            'rechargecard_list' => $rechargecard_list,
            'exportList' => \Yii::$app->serializer->encode($exportList)
        ]);
    }
     ...
     * 会员卡号删除
     */
    public function actionSetCarddel($id = null)
    {


        $cardmi = Cardmi::findOne(['id' => $id, 'store_id' => $this->store->id]);
        if (!$cardmi) {
            return [
                'code' => 1,
                'msg' => '卡密信息不存在',
     ...
     * 会员卡号发放
     */
    public function actionSetCardfa($id = null)
    {
     ...
     * 会员卡号激活
     */
    public function actionSetCardji($id = null)
    {
        ...
        $cardmi->is_use = $cardmi->is_use?0:1;
        if ($cardmi->save()) {
     ...
     * 批量设置
     */
    public function actionRechargeCardBatch()
    {
        ...
        if ($get['type'] == 0) { //批量发放
            $res = Cardmi::updateAll(['is_fa' => 1], $condition);
//            print_r($res);die;
        } elseif ($get['type'] == 1) { //批量激活
            $res = Cardmi::updateAll(['is_use' => 1], $condition);
        } elseif ($get['type'] == 2) { //批量删除
            $res = Cardmi::updateAll(['is_delete' => 1], $condition);
        } elseif ($get['type'] == 3) { //批量不激活
            $res = Cardmi::updateAll(['is_use' => 0], $condition);
        } elseif ($get['type'] == 4) { //批量不发放
            $res = Cardmi::updateAll(['is_fa' => 0], $condition);
        }
     ...
     * 会员等级编辑
     */
    public function actionRechargeCardEdit($id = null)
    {
        $rechargecard = RechargeCard::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
//        echo "<pre>";
//        print_r($rechargecard);die;
        if (!$rechargecard) {
            $rechargecard = new RechargeCard();
        }
        $store = Store::findOne(['id' => $this->store->id]);
        if (\Yii::$app->request->isAjax) {
            $form = new RechargeCardForm();
            $post = \Yii::$app->request->post();

            $form->store_id = $this->store->id;
            $form->model = $rechargecard;
            $form->attributes = $post;
            return $form->save();
        }

        return $this->render('recharge-card-edit', [
     ...
     * 会员充值记录
     */
    public function actionRechargelog($keyword = null)
    {
        $dateStart = \Yii::$app->request->get('date_start');
        $dateEnd = \Yii::$app->request->get('date_end');
        $user_id = \Yii::$app->request->get('user_id');
        ...
        $query = Cardlog::find()->alias('cl')->where([
            'cl.store_id' => $this->store_id
        ]) ->leftJoin(User::tableName() . ' u', 'u.id=cl.user_id ')
            ->leftJoin(Cardmi::tableName() . ' cm', 'cm.id = cl.use_card_id' )
            ->leftJoin(Level::tableName() . ' l', 'cm.level_type=l.id');
        if ($keyword) {
            $query->andWhere(['like', 'u.nickname', $keyword]);
        }
        if ($dateStart) {
            $query->andWhere(['>', 'cl.use_time', strtotime($dateStart)]);
        }
        if ($dateEnd) {
            $query->andWhere(['<', 'cl.use_time', strtotime($dateEnd + 1)]);
        }
        if($user_id){
        ...
        $p = new Pagination(['totalCount' => $count, 'pageSize' => 20]);
        $list = $query->select([
            'u.nickname', 'cl.*','l.name as lname'
        ])->limit($p->limit)->offset($p->offset)->orderBy('cl.id DESC')->asArray()->all();
    ...
    //商品上下架
    public function actionUpdateStatus($id = 0, $type = 'down')
    {
        if ($type == 'start') {
            $user = User::findOne(['id' => $id, 'is_delete' => 0, 'store_id' => $this->store->id]);
     ...
     * 会员充值记录
     */
    public function actionJifenlog($keyword = null)
    {
        $dateStart = \Yii::$app->request->get('date_start');
        $dateEnd = \Yii::$app->request->get('date_end');
        $user_id = \Yii::$app->request->get('user_id');
        ...
        $query = Jifenlog::find()->alias('cl')->where([
            'cl.store_id' => $this->store_id
        ]) ->leftJoin(User::tableName() . ' u', 'u.id=cl.user_id ')
            ->leftJoin(Cardmi::tableName() . ' cm', 'cm.id = cl.use_card_id' )
            ->leftJoin(Level::tableName() . ' l', 'cm.level_type=l.id');
