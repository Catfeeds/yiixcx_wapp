...
//                    $newItem['order_status'] = $datum['is_confirm'] == 1 ? '已完成' : '进行中';
     ...
     * 用户信息导出
     */
    public function UsercardExportData($list)
    {
        ...
        foreach ($list as $item) {
            $arr = [];
            $arr['id'] = $item['id'];
            $arr['level_name'] = $item['r_name'];
            $arr['level_price'] = $item['r_price'];
            $arr['card_id'] = $item['card_id'];
            $arr['card_password'] = $item['card_password'];
            $arr['is_fa'] = $item['is_fa']?'已发放':'未发放';
            $arr['is_use'] = $item['is_use']?'已使用':'未使用';
            $arr['addtime'] = trim("\"\t" . date('Y-m-d H:i:s', $item['addtime']) . "\"");
//            $arr['usetime'] = trim("\"\t" .$item['usetime']?date('Y-m-d H:i:s', $item['usetime']):'' . "\"");
